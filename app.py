...
from tkinter import Button, Frame, messagebox, scrolledtext, simpledialog, ttk
...
class ScriptRunnerApp:
    def __init__(self, root):
        ...
        self.project_combo.bind(
            "<<ComboboxSelected>>",
            lambda e: self.select_project(self.project_var.get()),
        )
        ...
        Button(
            self.project_frame, text="Refresh", command=self.refresh_projects, width=6
        ).pack(side="left", padx=(0, 6))

        # Current project label
        self.current_label_var = tk.StringVar(value="(no project selected)")
        ttk.Label(self.project_frame, textvariable=self.current_label_var).pack(
            side="left", padx=(10, 6)
        )
        ...
        btn_names = [
            ...
            ("Train & batch classify", "scripts/classify_track.py"),
        ...
        self.output_area = scrolledtext.ScrolledText(
            self.root, wrap=tk.WORD, state="normal", height=22
        )
    ...
    def is_settings_populated(self, project_path: Path) -> bool:
        ...
        try:
            ...
            return nonempty_list_field(
                d.get("primary_motion_classes", fallback="0")
    ...
    def update_button_states(self):
        ...
        settings_script = "scripts/settings_gui.py"
    ...
    def select_project(self, project_name):
        ...
        if not proj_path.exists():
            messagebox.showerror(
                "Project not found", f"Project '{project_name}' not found."
            )
            self.refresh_projects()
            return
    ...
    def create_new_project(self):
        name = simpledialog.askstring(
            "New project",
            "Enter new project name (alphanumeric, - and _ allowed):",
            parent=self.root,
        )
        if not name:
            return
        ...
        if any(c in name for c in r'\/:*?"<>|'):
            messagebox.showerror(
                "Invalid name", "Project name contains invalid characters."
            )
            return
...
"""
            ...
            messagebox.showerror(
                "Error creating project", f"Failed to create project '{name}':\n{e}"
            )
            return
    ...
    def run_script(self, script_name):
        if self.current_project is None:
            messagebox.showwarning(
                "No project", "Please select or create a project first."
            )
            return
        # run in background thread
        threading.Thread(
            target=self.execute_script, args=(script_name,), daemon=True
        ).start()
    ...
    def execute_script(self, script_name):
        ...
        try:
            proc = subprocess.Popen(
                [sys.executable, "-u", str(script_path), str(self.current_project)],
        ...
        threading.Thread(
            target=self.read_stream, args=(proc.stdout, "stdout"), daemon=True
        ).start()
        threading.Thread(
            target=self.read_stream, args=(proc.stderr, "stderr"), daemon=True
        ).start()
        code = proc.wait()
        if code == 0:
    ...
    def update_output(self):
        while not self.output_queue.empty():
            ...
            if buf.endswith(b"\r"):
                ...
                if line_plain.startswith("[ WARN:0@") or line_plain.startswith(
                    "[ERROR:0@"
                ):  # don't print OpenCV camera searching errors
                    continue
                else:
                    if is_progress_line(line_plain):
                    ...
                    # ~ self.last_progress_global = None
                        ...
                        if (
                            self.last_progress_global is not None
                            and line_stripped == self.last_progress_global
                        ):
                            self.last_progress_global = None
                        else:
                            rd.write(line_plain + "\n")
