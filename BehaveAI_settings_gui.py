...
class ClassRow(ttk.Frame):
    def __init__(
        ...
        **kwargs,
        ...
        self.color_btn = tk.Button(
            self, text="Choose", command=self.pick_color, width=8
        )
        self._update_btn_color()

        self.ignore_secondary_cb = ttk.Checkbutton(
            self,
        ...
        # layout columns
        col = 0
        self.label_entry.grid(row=0, column=col, sticky="w", padx=(0, 6))
        col += 1
        self.hotkey_entry.grid(row=0, column=col, sticky="w", padx=(0, 6))
        col += 1
        self.color_btn.grid(row=0, column=col, sticky="w", padx=(0, 6))
        col += 1
    ...
    def pick_color(self):
        ...
        try:
            initial = (
                f"#{self.r_var.get():02x}{self.g_var.get():02x}{self.b_var.get():02x}"
            )
        except Exception:
            initial = None
...
class ClassListEditor(ttk.Frame):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        try:
            ...
            ttk.Label(self, text=title, font=font_bold).grid(
        ...
        ttk.Button(btn_frame, text="Clear", command=self.clear).grid(
    ...
    def add_row(self, label="", hotkey=None, color=None, ignore_secondary=False):
        ...
        if hotkey is None or hotkey == "":
            assigned_hotkey, assigned_color_from_counter = (
                self._pick_defaults_and_advance_counter()
            )
        ...
        self._create_row(
            label,
            assigned_hotkey or "",
            assigned_color or DEFAULT_FALLBACK_COLOR,
            ignore_secondary,
        )
        if self.on_change:
            self.on_change()
...
class SettingsEditorApp(tk.Tk):
    ...
    def _validate_hotkeys(self):
        ...
        for key, editor in self.class_editors.items():
            for label, hotkey, _, _ in editor.get():
                if not hotkey:
                    errors.append(f"Class '{label}' does not have a hotkey assigned.")
                    continue
                ...
                if hk in used:
                    errors.append(
                        f"Hotkey '{hotkey}' is used by both '{used[hk]}' and '{label}'."
                    )
                else:
                    used[hk] = label
    ...
    def _build_ui(self):
        ...
        for key, title in CLASS_GROUPS:
            ...
            editor = ClassListEditor(
                tab1,
                title=title,
                on_change=self._set_dirty,
        ...
        ttk.Checkbutton(
            tab1,
            text="Motion blocks static",
            variable=self.motion_blocks_static_var,
            command=self._set_dirty,
        ).pack(anchor="w", pady=(8, 0))
        self.static_blocks_motion_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(
            tab1,
            text="Static blocks motion",
            variable=self.static_blocks_motion_var,
            command=self._set_dirty,
        ).pack(anchor="w")
        ...
        def _browse_dir(var):
            path = filedialog.askdirectory(
                initialdir=var.get() or self.project_dir, title="Select directory"
            )
            if path:
                var.set(path)
                self._set_dirty()
        ...
        def _path_row(parent, label, var, row):
            ...
            ttk.Button(parent, text="Select…", command=lambda: _browse_dir(var)).grid(
        ...
        ttk.Checkbutton(
            tab2,
            text="Chromatic tail only",
            variable=self.chromatic_tail_only_var,
            command=self._set_dirty,
        ).grid(row=1, column=0, sticky="w", padx=8, pady=(6, 0))

        ttk.Label(tab2, text="Green decay (expA)").grid(
            row=2, column=0, sticky="w", padx=8, pady=(6, 0)
        )
        ...
        ttk.Spinbox(
            ...
            to=0.99,
            increment=0.01,
            textvariable=self.expA_var,
            width=6,
            command=self._set_dirty,
        ).grid(row=2, column=1, sticky="w", padx=8)

        ttk.Label(tab2, text="Red decay (expB)").grid(
        ...
        ttk.Spinbox(
            ...
            to=0.99,
            increment=0.01,
            textvariable=self.expB_var,
            width=6,
            command=self._set_dirty,
        ).grid(row=3, column=1, sticky="w", padx=8)

        ttk.Label(tab2, text="Lum weight").grid(
        ...
        ttk.Label(tab3, text="Validation frequency").grid(
            row=0, column=0, sticky="w", padx=8, pady=(8, 0)
        )
        ...
        ttk.Spinbox(
            ...
            to=1.0,
            increment=0.01,
            textvariable=self.val_frequency_var,
            width=6,
            command=self._set_dirty,
        ).grid(row=0, column=1, sticky="w", padx=8)

        ttk.Label(tab3, text="Primary classifier").grid(
            row=1, column=0, sticky="w", padx=8, pady=(8, 0)
        )
        self.primary_classifier_var = tk.StringVar(value="yolo26n.pt")
        ttk.Combobox(
            tab3, values=CLASSIFIER_OPTIONS, textvariable=self.primary_classifier_var
        ).grid(row=1, column=1, sticky="w", padx=8, pady=(8, 0))
        self.primary_classifier_var.trace_add("write", lambda *a: self._set_dirty())

        ttk.Label(tab3, text="Primary epochs").grid(
            row=2, column=0, sticky="w", padx=8, pady=(6, 0)
        )
        ...
        ttk.Spinbox(
            ...
            from_=1,
            to=10000,
            textvariable=self.primary_epochs_var,
            width=8,
            command=self._set_dirty,
        ).grid(row=2, column=1, sticky="w", padx=8)

        ttk.Label(tab3, text="Secondary classifier").grid(
            row=3, column=0, sticky="w", padx=8, pady=(6, 0)
        )
        ...
        ttk.Combobox(
            tab3, values=secondary_opts, textvariable=self.secondary_classifier_var
        ).grid(row=3, column=1, sticky="w", padx=8)
        self.secondary_classifier_var.trace_add("write", lambda *a: self._set_dirty())

        ttk.Label(tab3, text="Secondary epochs").grid(
            row=4, column=0, sticky="w", padx=8, pady=(6, 0)
        )
        ...
        ttk.Spinbox(
            ...
            from_=1,
            to=10000,
            textvariable=self.secondary_epochs_var,
            width=8,
            command=self._set_dirty,
        ).grid(row=4, column=1, sticky="w", padx=8)
        ...
        ttk.Label(tab4, text="Match distance thresh").grid(
            row=0, column=0, sticky="w", padx=8, pady=(8, 0)
        )
        ...
        ttk.Label(tab4, text="Delete after missed").grid(
        ...
        ttk.Spinbox(
            ...
            from_=1,
            to=10000,
            textvariable=self.delete_after_var,
            width=8,
            command=self._set_dirty,
        ).grid(row=1, column=1, sticky="w", padx=8)

        ttk.Label(tab4, text="Centroid merge thresh").grid(
            row=2, column=0, sticky="w", padx=8, pady=(6, 0)
        )
        ...
        ttk.Spinbox(
            ...
            from_=1,
            to=10000,
            textvariable=self.centroid_merge_var,
            width=8,
            command=self._set_dirty,
        ).grid(row=2, column=1, sticky="w", padx=8)

        ttk.Label(tab4, text="IOU thresh (overlap required to merge)").grid(
            row=3, column=0, sticky="w", padx=8, pady=(6, 0)
        )
        ...
        ttk.Entry(tab4, textvariable=self.kalman_pos_var).grid(
            row=5, column=1, sticky="w", padx=8
        )
        ...
        self.save_btn = ttk.Button(
            bottom, text="Save", command=self.on_save, state="normal"
        )
    ...
    def load_ini(self, path):
        if not os.path.exists(path):
            if messagebox.askyesno(
                "Create new",
                f"{path} not found. Create a new settings file at this path?",
            ):
                open(path, "w").close()
            else:
                return
        ...
        for key, _title in CLASS_GROUPS:
            ...
            for i, label in enumerate(cls):
                ...
                editor.add_row(
                    label=label, hotkey=hot, color=col, ignore_secondary=is_ignored
                )
        ...
        self.motion_blocks_static_var.set(
            self._str_to_bool(d.get("motion_blocks_static", fallback="true"))
        )
        self.static_blocks_motion_var.set(
            self._str_to_bool(d.get("static_blocks_motion", fallback="false"))
        )

        # motion tab
        self.strategy_var.set(d.get("strategy", fallback="exponential"))
        self.chromatic_tail_only_var.set(
            self._str_to_bool(d.get("chromatic_tail_only", fallback="false"))
        )
        ...
        self._loaded_motion_settings = {
            ...
            "chromatic_tail_only": str(
                d.get("chromatic_tail_only", fallback="false")
            ).lower(),
            ...
            "motion_blocks_static": str(
                d.get("motion_blocks_static", fallback="false")
            ).lower(),
            "static_blocks_motion": str(
                d.get("static_blocks_motion", fallback="false")
            ).lower(),
        }

        # model type
        self.val_frequency_var.set(float(d.get("val_frequency", fallback="0.2")))
        self.primary_classifier_var.set(
            d.get("primary_classifier", fallback="yolo26n.pt")
        )
        self.primary_epochs_var.set(int(d.get("primary_epochs", fallback="100")))
        self.secondary_classifier_var.set(
            d.get("secondary_classifier", fallback="yolo26n-cls.pt")
        )
        ...
        try:
            ...
            try:
                primary_static_classes = [
                    label
                    for (label, _, _, _) in self.class_editors["primary_static"].get()
                ]
                primary_motion_classes = [
                    label
                    for (label, _, _, _) in self.class_editors["primary_motion"].get()
                ]
            except Exception:
                primary_static_classes = []
                primary_motion_classes = []
            ...
            messagebox.showwarning(
                "YAML write error", f"Failed to write dataset YAMLs: {e}"
            )
    ...
    def _motion_settings_changed(self):
        ...
        for k, v in curr.items():
            if k not in self._loaded_motion_settings or str(
                self._loaded_motion_settings[k]
            ) != str(v):
                return True
        return False
    ...
    def on_save(self):
        ...
        if hotkey_errors:
            messagebox.showwarning("Invalid hotkeys", "\n".join(hotkey_errors))
            return

        primary_error = self._validate_primary_classes()
        if primary_error:
            messagebox.showwarning("Missing primary class", primary_error)
            return
        ...
        # viewing
        new_default["motion_blocks_static"] = str(
            self.motion_blocks_static_var.get()
        ).lower()
        ...
        new_default["chromatic_tail_only"] = str(
            self.chromatic_tail_only_var.get()
        ).lower()
            ...
            if should_prompt_regen:
                ...
                if ans:
                    ...
                    if backed:
                        ...
                        print(
                            "No primary/secondary motion model directories found to back up."
                        )
                    ...
                    if ok:
                        messagebox.showinfo(
                            ...
                            "Dataset regeneration finished successfully.",
                        )
                    else:
    ...
    def on_cancel(self):
        if self.dirty:
            if not messagebox.askyesno(
                "Discard changes?", "There are unsaved changes. Discard and exit?"
            ):
                return
        self.destroy()
