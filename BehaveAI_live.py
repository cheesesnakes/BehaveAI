...
if len(sys.argv) > 1:
    ...
    config_path = (
        os.path.join(arg, "BehaveAI_settings.ini") if os.path.isdir(arg) else arg
    )
else:
...
def resolve_project_path(value, fallback):
    ...
    return (
        ...
        else os.path.normpath(os.path.join(project_dir, value))
    )
    ...
    try:
        ...
        if os.path.exists("/etc/os-release"):
            with open("/etc/os-release", "r", encoding="utf-8", errors="ignore") as f:
                txt = f.read().lower()
                if any(
                    tok in txt for tok in ("raspbian", "raspberry", "raspberry pi os")
                ):
                    return True
    except Exception:
        pass
...
try:
    ...
    primary_motion_hotkeys = [
        k.strip() for k in config["DEFAULT"]["primary_motion_hotkeys"].split(",")
    ]
    ...
    secondary_motion_hotkeys = [
        k.strip() for k in config["DEFAULT"]["secondary_motion_hotkeys"].split(",")
    ]
    ...
    primary_static_hotkeys = [
        k.strip() for k in config["DEFAULT"]["primary_static_hotkeys"].split(",")
    ]
    ...
    secondary_static_hotkeys = [
        k.strip() for k in config["DEFAULT"]["secondary_static_hotkeys"].split(",")
    ]
...
def iou(box1, box2):
    ...
    area1 = (
        ...
        if box1[2] > box1[0] and box1[3] > box1[1]
        else 0
    )
    area2 = (
        (box2[2] - box2[0]) * (box2[3] - box2[1])
        if box2[2] > box2[0] and box2[3] > box2[1]
        else 0
    )
...
class CameraProcessor:
    ...
    def set_show_detections_in_recording(self, flag: bool):
        ...
        print(
            f"Show detections in recordings set to: {self.show_detections_in_recording}"
        )

    def _maybe_load_models(self):
        # Primary models
        try:
            if len(primary_static_classes) > 0 and os.path.exists(
                primary_static_model_path
            ):
                # ~ self.model_static = YOLO(primary_static_model_path)
                if use_ncnn == "true":
                    self.model_static = load_model_with_ncnn_preference(
            ...
            if len(primary_motion_classes) > 0 and os.path.exists(
                primary_motion_model_path
            ):
                if use_ncnn == "true":
                    self.model_motion = load_model_with_ncnn_preference(
            ...
            for p in primary_classes:
                ...
                try:
                    ...
                    if os.path.exists(model_path):
                        ...
                        if use_ncnn == "true":
                            self.secondary_static_models[p] = (
                                load_model_with_ncnn_preference(model_path, "classify")
                            )
                            print(
                                f"Loaded secondary static NCNN model for primary '{p}'"
                            )
                        else:
                            self.secondary_static_models[p] = YOLO(model_path)
                            print(
                                f"Loaded secondary static YOLO model for primary '{p}'"
                            )
                ...
                try:
                    ...
                    if os.path.exists(model_path):
                        ...
                        if use_ncnn == "true":
                            self.secondary_motion_models[p] = (
                                load_model_with_ncnn_preference(model_path, "classify")
                            )
                            print(
                                f"Loaded secondary motion NCNN model for primary '{p}'"
                            )
                        else:
                            self.secondary_static_models[p] = YOLO(model_path)
                            print(
                                f"Loaded secondary motion YOLO model for primary '{p}'"
                            )
                except Exception as e:
    ...
    def set_resolution(self, wh_tuple):
        ...
        if getattr(self, "cap", None) is not None and self.cap.isOpened():
            ...
            print(
                f"set_resolution: desired set to {w}x{h}; will apply when camera opens"
            )

    def toggle_classifier(self, enable: bool):
        self.classifier_enabled = bool(enable)
        if self.classifier_enabled and (
            self.model_static is None and self.model_motion is None
        ):
            self._maybe_load_models()
    ...
    def stop(self):
        ...
        self.running = False
        try:
            if getattr(self, "manual_writer", None):
                self.manual_writer.release()
                self.manual_writer = None
        except Exception:
            pass
        try:
            if getattr(self, "detection_writer", None):
                self.detection_writer.release()
                self.detection_writer = None
        except Exception:
            pass
    ...
    def set_camera(self, index):
        with self.frame_lock:
            if self.running:
                print(
                    "set_camera: processor already running; camera index change ignored."
                )
                return
    ...
    def _open_camera(self, timeout=5.0, retry_interval=0.2):
        ...
        while not self.stop_event.is_set() and time.time() < deadline:
            try:
                ...
                # Picamera2 path
                if (
                    str(self.camera_index) == "picamera"
                    and IS_RPI
                    and PICAMERA2_AVAILABLE
                ):
                    try:
                        res = (
                            self.desired_resolution
                            if getattr(self, "desired_resolution", None)
                            else (640, 480)
                        )
                        # choose requested fps: prefer processor.desired_fps if >0, else try 30
                        fps_req = (
                            self.desired_fps
                            if getattr(self, "desired_fps", 0.0)
                            else 30.0
                        )
                        # create wrapper and start (will raise on repeated failure)
                        self.cap = Picamera2Wrapper(
                            resolution=tuple(map(int, res)),
                            fps=int(round(fps_req)),
                            retries=4,
                            retry_delay=0.5,
                        )
                        self.cap.start()
                        if self.cap.isOpened():
                            print(
                                f"_open_camera: picamera opened (requested {res[0]}x{res[1]} @ {fps_req}fps)"
                            )
    ...
    def _start_manual_writer(self, w, h, fps):
        if not hasattr(self, "manual_writer_path"):
            return
        fourcc = cv2.VideoWriter_fourcc(*"mp4v")
        self.manual_writer = cv2.VideoWriter(
            self.manual_writer_path, fourcc, fps, (w, h)
        )
    ...
    def _run_loop(self):
        ...
        cv2.namedWindow(
            self.show_window_name,
            cv2.WINDOW_NORMAL | cv2.WINDOW_GUI_NORMAL | cv2.WINDOW_KEEPRATIO,
        )
        ...
        while not self.stop_event.is_set():
            ...
            if prev_frames is None:
                ...
                tnow = time.time()
                self.frame_timestamps.append(tnow)
            ...
            if self.classifier_enabled:
                ...
                if primary_static_classes[0] != "0" and self.model_static is not None:
                    try:
                        results_static = self.model_static.predict(
                ...
                if primary_motion_classes[0] != "0" and self.model_motion is not None:
                    try:
                        results_motion = self.model_motion.predict(
                    ...
                    if dist < centroid_merge_thresh or overlap > iou_thresh:
                        ...
                        if (
                            ...
                            if det["primary_conf"] > md.get("primary_conf", 0):
                                md["primary_class_combined"] = md.get(
                                    "primary_class", ""
                                )
            ...
            for det in merged_detections:
                ...
                if source == "static":
                    ...
                    det["primary_motion_class"] = det.get("primary_class_combined", "")
                    det["primary_motion_conf"] = det.get("primary_conf_combined", 0.0)
                else:
                    det["primary_motion_class"] = primary_class
                    det["primary_motion_conf"] = primary_conf
                    det["primary_static_class"] = det.get("primary_class_combined", "")
                    det["primary_static_conf"] = det.get("primary_conf_combined", 0.0)
                ...
                if hierarchical_mode:
                    ...
                    # Safe crop bounds
                    h_img, w_img = (
                        crop_img.shape[:2] if crop_img is not None else (0, 0)
                    )
                    x1c = max(0, min(w_img - 1, int(x1)))
                    x2c = max(0, min(w_img - 1, int(x2)))
                    y1c = max(0, min(h_img - 1, int(y1)))
                    y2c = max(0, min(h_img - 1, int(y2)))
                    crop = None
                    ...
                    if sec_model and crop is not None and crop.size > 0:
                        try:
                            ...
                            if (
                                hasattr(sec_results[0], "probs")
                                and sec_results[0].probs is not None
                            ):
                    ...
                    if source == "static":
                        ...
                        det["secondary_motion_class"] = ""
                        det["secondary_motion_conf"] = 0.0
                    else:
                        det["secondary_motion_class"] = secondary_class
                        det["secondary_motion_conf"] = secondary_conf
                        det["secondary_static_class"] = ""
                        det["secondary_static_conf"] = 0.0
            ...
            for idx, det in enumerate(processed_detections):
                ...
                primary_col = (
                    primary_colors[primary_classes.index(primary_cls)]
                    if primary_cls in primary_classes
                    else (0, 255, 0)
                )
                ...
                if hierarchical_mode:
                    ...
                    if primary_cls in ignore_secondary:
                        ...
                        cv2.rectangle(
                            frame,
                            (x1 - line_thickness, y1 - lh - line_thickness * 4),
                            (x1 + lw + line_thickness * 2, y1),
                            (0, 0, 0),
                            -1,
                        )
                        cv2.rectangle(
                        ...
                        cv2.rectangle(
                            frame,
                            (x1 - outer_t, y1 - outer_t),
                            (x2 + outer_t, y2 + outer_t),
                            primary_col,
                            outer_t,
                        )
            ...
            cv2.rectangle(
                frame,
                (0, 0),
                (lw + line_thickness * 4, lh + line_thickness * 4),
                (0, 0, 0),
                -1,
            )
            cv2.putText(
                frame,
                label,
                (line_thickness * 2, lh + line_thickness * 2),
                cv2.FONT_HERSHEY_SIMPLEX,
                font_size,
                text_color,
                line_thickness,
            ...
            record_frame = (
                display_frame if self.show_detections_in_recording else raw_frame
            )
            ...
            if has_detections:
                ...
                if (
                    ...
                    and (nowt - self.detection_last_seen) > self.detection_stop_seconds
                ):
        ...
        if self.detection_writer:
            ...
            self.detection_writer = None
        try:
            cv2.destroyWindow(self.show_window_name)
        except Exception:
            pass
    ...
    def _draw_record_icon(self, img, manual_active: bool, detection_active: bool):
        ...
        dot_radius = 8
        label = "REC"
        if manual_active and detection_active:
            label = "REC M+D"
        elif manual_active:
            label = "REC M"
        ...
        rect_w = dot_radius * 2 + 6 + text_w + 6
        rect_h = max(text_h + 6, dot_radius * 2 + 6)
        x1 = w - margin - rect_w
        y1 = margin
        x2 = w - margin
        y2 = y1 + rect_h
        ...
        dot_cx = x1 + 6 + dot_radius
        dot_cy = y1 + rect_h // 2
        cv2.circle(img, (dot_cx, dot_cy), dot_radius, (0, 0, 255), -1)
        cv2.circle(
            img, (dot_cx + 1, dot_cy - 1), max(1, dot_radius // 3), (255, 255, 255), -1
        )
        text_x = dot_cx + dot_radius + 6
        text_y = dot_cy + text_h // 2 - 2
        cv2.putText(
            img,
            label,
            (text_x, text_y),
...
class Picamera2Wrapper:
    ...
    def configure_and_start(self):
            ...
            try:
                ...
                try:
                    ...
                    last_exc = RuntimeError(
                        f"Failed to recreate Picamera2 after attempt {attempt}: {e}"
                    )
                    break
                import time
        ...
        raise RuntimeError(
            f"Picamera2 configure/start failed after {self.retries} attempts: {last_exc}"
        ) from last_exc
...
class ControlGUI:
    def __init__(self, root, processor: CameraProcessor):
        ...
        self.classifier_chk = ttk.Checkbutton(
            frm,
            text="Enable classifier",
            variable=self.classifier_var,
            command=self._toggle_classifier,
        )
        self.classifier_chk.grid(row=1, column=2, sticky="w")

        self.detect_rec_var = tk.BooleanVar(value=False)
        self.detect_chk = ttk.Checkbutton(
            frm,
            text="Record on detection (output)",
            variable=self.detect_rec_var,
            command=self._toggle_detect_recording,
        )
        ...
        self.show_det_chk = ttk.Checkbutton(
            frm,
            text="Show detections in recordings",
            variable=self.show_det_var,
            command=self._toggle_show_detections,
        )
        ...
        rb_static.grid(row=3, column=1, sticky="w")
        rb_motion.grid(row=3, column=2, sticky="w")
        rb_disabled.grid(row=3, column=3, sticky="w")

        ttk.Label(frm, text="FPS:").grid(row=4, column=0, sticky="w")
        self.fps_label = ttk.Label(frm, text="0.0")
        self.fps_label.grid(row=4, column=1, sticky="w")

        ttk.Label(frm, text="Throttle FPS (0 = off):").grid(row=5, column=0, sticky="w")
        self.throttle_var = tk.DoubleVar(value=0.0)
        self.throttle_spin = tk.Spinbox(
            frm,
            from_=0,
            to=120,
            increment=1,
...
def main():
    ...
    cams = scan_cameras(8)
    if cams:
        gui.cam_combo.set(cams[0])
        processor.set_camera(cams[0])
