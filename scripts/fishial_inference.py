# -*- coding: utf-8 -*-
"""
cloned from: https://storage.googleapis.com/fishial-ml-resources/classification_model_v0.10.2.zip
...
Use ``embed()`` to extract raw normalized embeddings without classification.
Use ``predict()`` to get top-k predictions for a specific scoring method, with optional class masking. Defaults to ``natural_centroid`` for best overall performance and robustness.
Use ``predict_single()`` for a simplified API when only one image is being processed (returns a single FishResult instead of a list).
Use ``predict_high_accuracy()`` for TTA-augmented predictions with the "natural_centroid" method, which applies photometric augmentations and averages the resulting scores for improved accuracy at the cost of increased latency.
"""
...
# InferenceConfig defaults (can be overridden per-engine instance)
MAX_UNIQUE_CLASSES = 5
...
class InferenceConfig:
    ...
    max_unique_classes: int = MAX_UNIQUE_CLASSES
    return_emb: bool = RETURN_EMBEDDING
    k_centers: int = K_CENTERS
...
@lru_cache(maxsize=8)
def build_transform(input_size: tuple = DEFAULT_INPUT_SIZE) -> A.Compose:
...
@lru_cache(maxsize=8)
def build_tta_transform(input_size: tuple = DEFAULT_INPUT_SIZE) -> A.Compose:
    """
...
class FishInferenceEngine:
    ...
    def __init__(
        ...
        input_size: tuple = DEFAULT_INPUT_SIZE,  # (H, W)
        device: str = DEVICE,
        config: Optional[InferenceConfig] = None,
    ) -> None:
        logger.info(
            f"Initializing FishInferenceEngine on device '{device}' with input_size={input_size}"
        )
        ...
        logger.info(
            f"Loading natural centroids and class mapping from: {natural_gallery_path}"
        )
        try:
            nat_data = torch.load(
                natural_gallery_path, map_location=device, weights_only=False
            )
            logger.debug("Successfully loaded natural gallery file.")
        except Exception as e:
            logger.error(
                f"Failed to load natural gallery from {natural_gallery_path}: {e}"
            )
            raise
        ...
        if not self.class_mapping:
            logger.warning(
                "No 'labels_keys' found in the natural gallery file. Predictions will use raw class IDs."
            )
        else:
        ...
        with torch.no_grad():
            ...
            try:
                if hasattr(self.model, "arcface_head"):
                    ...
                    logger.warning(
                        "ArcFace head not found in TorchScript model — arcface_centroid disabled."
                    )
            except Exception as e:
            ...
            if "labels" not in nat_data:
                raise KeyError(
                    "Natural gallery must contain 'labels' aligned with centroids."
                )
            ...
            logger.info(
                f"Engine initialized. Centroids loaded: {self._natural_centroids.shape[0]}"
            )
    ...
    def from_bundle(
        ...
        try:
            ...
            if hasattr(model, "class_mapping_json_bytes"):
                ...
                logger.info(
                    f"Loaded class mapping: {len(engine.class_mapping)} classes"
                )
            else:
    ...
    def _crop_horizontal(
        ...
        try:
            ...
            canvas[y_offset : y_offset + tight_h, x_offset : x_offset + tight_w] = (
                tight_crop
            )
            ...
            logger.warning(
                f"_crop_horizontal failed ({type(e).__name__}: {e}); falling back to bbox crop."
            )
            return self._crop_fallback(image, bbox)
    ...
    def _get_arcface_centroid_scores(
        ...
        B = emb_norm.shape[0]
        logger.debug(
            f"arcface_centroid scoring: batch_size={B}, embeddings shape={emb_norm.shape}"
        )
        ...
        logger.debug(
            f"Similarities matrix: {similarities.shape}, range=[{similarities.min():.4f}, {similarities.max():.4f}]"
        )
        ...
        if valid_classes_mask is not None:
            logger.debug(
                f"Applying class mask: {valid_classes_mask.sum().item()}/{len(valid_classes_mask)} valid classes."
            )
            class_similarities = class_similarities.masked_fill(
                ~valid_classes_mask.to(self.device).unsqueeze(0), float("-inf")
            )

        values, predicted_classes = torch.topk(
            class_similarities, k=top_k, dim=1, largest=True
        )
    ...
    def _get_natural_centroid_scores(
        ...
        B = emb_norm.shape[0]
        logger.debug(
            f"natural_centroid scoring: batch_size={B}, embeddings shape={emb_norm.shape}"
        )

        similarities = torch.mm(
            emb_norm, self._natural_centroids.t()
        )  # (B, Num_Centroids)

        num_classes_from_mapping = (
            (max(self.class_mapping.keys()) + 1) if self.class_mapping else 0
        )
        ...
        logger.debug(
            f"Aggregating {self._natural_centroids.shape[0]} centroids into {num_classes} classes."
        )

        class_similarities = torch.full(
            (B, num_classes), float("-inf"), device=self.device
        )
        labels_expanded = self._natural_labels.unsqueeze(0).expand(B, -1)

        try:
            class_similarities.scatter_reduce_(
                1, labels_expanded, similarities, reduce="amax", include_self=False
            )
        except AttributeError:
            logger.warning(
                "torch.scatter_reduce_ unavailable; falling back to loop-based aggregation (slower)."
            )
    ...
    def _normalize_inputs(
        ...
        List[ImageLike], List[Optional[List[float]]], List[Optional[List[List[float]]]]
    ]:
        ...
        bbs = (
            [bboxes]
            if (single and bboxes is not None)
            else (bboxes if bboxes is not None else [None] * len(imgs))
        )
        pls = (
            [polys]
            if (single and polys is not None)
            else (polys if polys is not None else [None] * len(imgs))
        )
        return imgs, bbs, pls
    ...
    def _build_predictions(
        self, scores: np.ndarray, classes: np.ndarray
    ...
    def predict(
        ...
        if method not in ("arcface_logits", "arcface_centroid", "natural_centroid"):
            raise ValueError(
                f"Unknown method '{method}'. Use 'arcface_logits', 'arcface_centroid', or 'natural_centroid'."
            )
        ...
        if method == "arcface_logits":
            ...
            scores, classes = self._get_arcface_centroid_scores(
                emb_t, cfg.max_unique_classes, valid_classes_mask
            )
        elif method == "natural_centroid":
            scores, classes = self._get_natural_centroid_scores(
                emb_t, cfg.max_unique_classes
            )
    ...
    def predict_all_methods(
        ...
        if self._arcface_centroids is not None:
            ...
            logger.debug(
                "predict_all_methods: no ArcFace centroids; arcface_centroid omitted"
            )
    ...
    def predict_high_accuracy(
        ...
        logger.debug(
            f"TTA Forward Pass: Batch shape = {batch.shape} ({len(tensors)} views)"
        )
        ...
        with torch.no_grad():
            ...
            if aggregation == "embedding_mean":
                ...
                if method == "arcface_centroid":
                    scores, classes = self._get_arcface_centroid_scores(
                        emb_t_norm, self.config.max_unique_classes, valid_classes_mask
                    )
                elif method == "natural_centroid":
                    scores, classes = self._get_natural_centroid_scores(
                        emb_t_norm, self.config.max_unique_classes
                    )
                else:  # arcface
                    mean_logits = logits_t.mean(dim=0, keepdim=True)
                    probs = F.softmax(mean_logits, dim=-1)
                    scores_t, classes_t = torch.topk(
                        probs, k=self.config.max_unique_classes, dim=1
                    )
                    scores, classes = scores_t.cpu().numpy(), classes_t.cpu().numpy()
                ...
                return FishResult(
                    top_k=top_k,
                    embedding=emb_t_norm[0].cpu().numpy()
                    if self.config.return_emb
                    else None,
                )
            ...
            # -----------------------------------------------------------------
                ...
                if method == "arcface_centroid":
                    scores, classes = self._get_arcface_centroid_scores(
                        emb_t, self.config.max_unique_classes, valid_classes_mask
                    )
                elif method == "natural_centroid":
                    scores, classes = self._get_natural_centroid_scores(
                        emb_t, self.config.max_unique_classes
                    )
                else:  # arcface
                    probs = F.softmax(logits_t, dim=-1)
                    scores_t, classes_t = torch.topk(
                ...
                top_k_results = []
                for cls_id, total_score in sorted_classes[
                    : self.config.max_unique_classes
                ]:
                ...
                return FishResult(
                    top_k=top_k_results,
                    embedding=mean_emb[0].cpu().numpy()
                    if self.config.return_emb
                    else None,
                )
    ...
    def warmup(self, num_iterations: int = 3) -> None:
        ...
        """
        logger.info(
            f"Warming up model for {num_iterations} iteration(s) on device '{self.device}'..."
        )

        # Create a dummy tensor of the expected input size
        dummy = torch.randn(1, 3, self.input_size[0], self.input_size[1]).to(
            self.device
        )
