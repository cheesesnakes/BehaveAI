...
primary_motion_colors = params.get("primary_motion_colors", [])
primary_motion_hotkeys = params.get("primary_motion_hotkeys", [])
secondary_motion_colors = params.get("secondary_motion_colors", [])
secondary_motion_hotkeys = params.get("secondary_motion_hotkeys", [])
primary_static_colors = params.get("primary_static_colors", [])
primary_static_hotkeys = params.get("primary_static_hotkeys", [])
secondary_static_colors = params.get("secondary_static_colors", [])
secondary_static_hotkeys = params.get("secondary_static_hotkeys", [])
...
if len(primary_motion_classes) > len(primary_motion_colors) or len(
    ...
    raise ValueError(
        "Primary motion classes, colours and hotkeys must match in configuration. Ensure colours are seprated by semicolons"
    )
if len(secondary_motion_classes) > len(secondary_motion_colors) or len(
    secondary_motion_classes
) != len(secondary_motion_hotkeys):
    raise ValueError(
        "Secondary motion classes, colours and hotkeys must match in configuration. Ensure colours are seprated by semicolons"
    )
if len(primary_static_classes) > len(primary_static_colors) or len(
    primary_static_classes
) != len(primary_static_hotkeys):
    raise ValueError(
        "Primary static classes, colours and hotkeys must match in configuration. Ensure colours are seprated by semicolons"
    )
if len(secondary_static_classes) > len(secondary_static_colors) or len(
    secondary_static_classes
) != len(secondary_static_hotkeys):
    raise ValueError(
        "Secondary static classes, colours and hotkeys must match in configuration. Ensure colours are seprated by semicolons"
    )
...
cursor_pos = (
    video_width // 2,
    video_height // 2,
)  # in VIDEO PIXELS (not canvas coords)
ix = iy = -1
boxes = []
grey_boxes = []
frame_updated = True
...
def build_window_title(basename):
    elements = [
        "BehaveAI Annotations (inspect):",
        basename,
        "ESC=quit BACKSPACE=clear u=undo ENTER=save SPACE=toggle view LEFT/RIGHT </> seek",
    ]
...
def load_item(idx):
    ...
    if scale_factor != 1.0:
        ...
        motion_img_cv = cv2.resize(
            motion_img_cv, (0, 0), fx=scale_factor, fy=scale_factor
        )
...
def draw_boxes(frame):
        ...
        if hierarchical_mode:
            ...
            if primary_classes[primary_cls] in ignore_secondary:
                ...
                cv2.rectangle(
                    ...
                    (x1, y1),
                    (x2, y2),
                    secondary_colors[secondary_cls],
                    line_thickness,
                )
                cv2.putText(
                    frame,
            ...
            cv2.rectangle(
                frame, (x1, y1), (x2, y2), primary_colors[primary_cls], line_thickness
            )
            cv2.putText(
                frame,
                label,
    ...
    for gx1, gy1, gx2, gy2 in grey_boxes:
        ...
        cv2.rectangle(
            overlay,
            (int(gx1 * disp_scale_factor), int(gy1 * disp_scale_factor)),
            (int(gx2 * disp_scale_factor), int(gy2 * disp_scale_factor)),
            (128, 128, 128),
            -line_thickness,
        )
...
def refresh_display():
    ...
    canvas = np.zeros(
        (
            video_height + bottom_bar_height,
            video_width + right_frame_width + line_thickness,
            3,
        ),
        dtype=original_frame.dtype,
    )
    ...
    if disp_scale_factor != 1.0:
        disp = cv2.resize(
            disp,
            None,
            fx=disp_scale_factor,
            fy=disp_scale_factor,
...
class DatasetInspectorTk:
    def __init__(self, root):
        ...
        self.canvas = tk.Canvas(
            self.left, bg="black", highlightthickness=0, width=canvas_w, height=canvas_h
        )
        ...
        self.status_label = tk.Label(
            self.bottom_frame, textvariable=self.status_var, anchor="w"
        )
